...
parameter "param_email" do
  ...
  type "string"
  category "Cloud"
  allowed_pattern /^cloud-summit-2020\+\w*@rightscale\.com$/
  description "Use cloud-summit-2020+UNIQUE@rightscale.com where UNIQUE is a unique value"
end
...
parameter "param_ou_lob_parent" do
  ...
  category "Cloud"
  min_length 2
  allowed_values "IT", "SharedServices", "Production", "Staging", "Development"
  default "IT"
end
  ...
  sub task_label:"Main", on_error: stop_debugging() do
    sub task_label:"Creating Cloud Account" do
      if $param_provider == "AWS"
        ...
        sub task_label:"Moving New Account to OU" do
          # Get Org Roots detail
          call aws_list_org_roots() retrieve $org_roots_response
          $root_id = $org_roots_response["body"]["Roots"][0]["Id"]

          # Get OU Ids
          $ou_id = "Unknown"
          call aws_list_ous($root_id) retrieve $ou_list_response
          $ous = $ou_list_response["body"]["OrganizationalUnits"]
          foreach $ou in $ous do
            if $ou["Name"] == $param_ou_lob_parent
              $ou_id = $ou["Id"]
            end
          end

          # Move Account to LOB OU
          if $ou_id != "Unknown"
            call aws_move_account($aws_account_id, $root_id, $ou_id) retrieve $move_account_response
          end
        end
    ...
    sub task_label:"Applying Corporate Governance" do
      sub task_label:"Enabling CMP" do
        # Generate CMP Access Token
        call cmp_find_account_number() retrieve $master_account_id
        call cmp_find_shard($master_account_id) retrieve $shard
        call cmp_generate_access_token($shard, $refresh_token_cred_name) retrieve $rs_access_token_response, $rs_access_token

        # Create CMP Child Account
        ...
        if $param_provider == "AWS"
          ...
          foreach $cloud_id in $cloud_ids do
            $cloud_account_creds = {}
            $cloud_account_creds["aws_account_number"] = $account_id
            $cloud_account_creds["aws_access_key_id"] = $iam_access_key
            $cloud_account_creds["aws_secret_access_key"] = $iam_secret_access_key
            call cmp_create_cloud_acct($shard, $child_account_id, $rs_access_token, $cloud_id, $cloud_account_creds) retrieve $create_cloud_acct_response
          end

          $cred_access = {}
          $cred_access["credential"] = {}
          $cred_access["credential"]["name"] = "AWS_ACCESS_KEY_ID"
          $cred_access["credential"]["value"] = $iam_access_key
          call cmp_create_credential($shard, $child_account_id, $rs_access_token, $cred_access) retrieve $cred_response

          $cred_sec_key = {}
          $cred_sec_key["credential"] = {}
          $cred_sec_key["credential"]["name"] = "AWS_SECRET_ACCESS_KEY"
          $cred_sec_key["credential"]["value"] = $iam_secret_access_key
          call cmp_create_credential($shard, $child_account_id, $rs_access_token, $cred_sec_key) retrieve $cred_response
          ...
          foreach $cloud_id in $cloud_ids do
            $cloud_account_creds = {}
            $cloud_account_creds["client_id"] = $azure_application_id
            $cloud_account_creds["client_secret"] = $azure_client_secret
            $cloud_account_creds["tenant_id"] = $azure_tenant_id
            $cloud_account_creds["subscription_id"] = $account_id
            call cmp_create_cloud_acct($shard, $child_account_id, $rs_access_token, $cloud_id, $cloud_account_creds) retrieve $create_cloud_acct_response
          end

          $cred_access = {}
          $cred_access["credential"] = {}
          $cred_access["credential"]["name"] = "AZURE_TENANT_ID"
          $cred_access["credential"]["value"] = $azure_tenant_id
          call cmp_create_credential($shard, $child_account_id, $rs_access_token, $cred_access) retrieve $cred_response

          $cred_sec_key = {}
          $cred_sec_key["credential"] = {}
          $cred_sec_key["credential"]["name"] = "AZURE_SUBSCRIPTION_ID"
          $cred_sec_key["credential"]["value"] = $account_id
          call cmp_create_credential($shard, $child_account_id, $rs_access_token, $cred_sec_key) retrieve $cred_response

          $cred_access = {}
          $cred_access["credential"] = {}
          $cred_access["credential"]["name"] = "AZURE_APPLICATION_ID"
          $cred_access["credential"]["value"] = $azure_application_id
          call cmp_create_credential($shard, $child_account_id, $rs_access_token, $cred_access) retrieve $cred_response

          $cred_access = {}
          $cred_access["credential"] = {}
          $cred_access["credential"]["name"] = "AZURE_APPLICATION_KEY"
          $cred_access["credential"]["value"] = $azure_application_key
          call cmp_create_credential($shard, $child_account_id, $rs_access_token, $cred_access) retrieve $cred_response
          ...
          foreach $cloud_id in $cloud_ids do
            $cloud_account_creds = {}
            $cloud_account_creds["project"] = $account_id
            $cloud_account_creds["client_email"] = $gcp_client_id
            $cloud_account_creds["private_key"] = $gcp_private_key
            call cmp_create_cloud_acct($shard, $child_account_id, $rs_access_token, $cloud_id, $cloud_account_creds) retrieve $create_cloud_acct_response
          end

          $cred_access = {}
          $cred_access["credential"] = {}
          $cred_access["credential"]["name"] = "GCE_PROJECT_ID"
          $cred_access["credential"]["value"] = $account_id
          call cmp_create_credential($shard, $child_account_id, $rs_access_token, $cred_access) retrieve $cred_response

          $cred_sec_key = {}
          $cred_sec_key["credential"] = {}
          $cred_sec_key["credential"]["name"] = "GCE_PLUGIN_ACCOUNT"
          $cred_sec_key["credential"]["value"] = $gcp_client_id
          call cmp_create_credential($shard, $child_account_id, $rs_access_token, $cred_sec_key) retrieve $cred_response

          $cred_sec_key = {}
          $cred_sec_key["credential"] = {}
          $cred_sec_key["credential"]["name"] = "GCE_PLUGIN_PRIVATE_KEY"
          $cred_sec_key["credential"]["value"] = $gcp_private_key
          call cmp_create_credential($shard, $child_account_id, $rs_access_token, $cred_sec_key) retrieve $cred_response

        end

        # Check for CMP User
        call cmp_check_user($cmp_org_id, $rs_access_token, $param_email) retrieve $check_user_response
        if $check_user_response["body"] == []
          # Create User
          $first = split($param_email, "@")[0]
          $last = "user"
          $o_temp_passwd = uuid()
          call cmp_create_user($default_company_name, $param_email, $first, $last, $o_temp_passwd, $default_company_phone) retrieve @user
          $rs_user_href = @user.href
        else
          # Get User HREF
          $rs_user_id = $check_user_response["body"][0]["id"]
          $rs_user_href = "/api/users/"+$rs_user_id
        end

        # Grant 'admin' Role to CMP User
        call cmp_create_permission($shard, $child_account_id, $rs_access_token, $rs_user_href, "admin") retrieve $permission_response
      end

      sub task_label:"Creating Default Networks" do

        # Create default network
        $cloud_href = map($map_cloud_regions, $param_provider, $network_region)
        $network_payload = {}
        $network_payload["network"] = {}
        $network_payload["network"]["cidr_block"] = $network_cidr
        $network_payload["network"]["name"] = $network_name
        $network_payload["network"]["cloud_href"] = $cloud_href
        if $param_provider == "AWS"
          $network_payload["network"]["instance_tenancy"] = "default"
        end

        call cmp_create_network($shard, $child_account_id, $rs_access_token, $network_payload) retrieve $create_network_response
        $network_href = $create_network_response["headers"]["Location"]
        call cmp_get_cm_resource($shard, $child_account_id, $rs_access_token, $network_href) retrieve $get_network_response
        $o_vpc_id = $get_network_response["body"]["resource_uid"]

        $subnet_ids = []

        foreach $subnet in $subnet_objects do
          $datacenter_name = $subnet["subnet_availability_zone"]
          call cmp_get_datacenter($shard, $child_account_id, $rs_access_token, $cloud_href, $datacenter_name) retrieve $get_datacenter_response
          $datacenter_href = select($get_datacenter_response["body"][0]["links"],{"rel":"self"})[0]["href"]
          $subnet_payload = {}
          $subnet_payload["subnet"] = {}
          $subnet_payload["subnet"]["cidr_block"] = $subnet["subnet_block"]
          $subnet_payload["subnet"]["network_href"] = $network_href
          $subnet_payload["subnet"]["name"] = $subnet["subnet_name"]
          $subnet_payload["subnet"]["datacenter_href"] = $datacenter_href
          call cmp_create_subnet($shard, $child_account_id, $rs_access_token, $cloud_href, $subnet_payload) retrieve $cmp_create_subnet_response
          $subnet_href = $cmp_create_subnet_response["headers"]["Location"]
          call cmp_get_cm_resource($shard, $child_account_id, $rs_access_token, $subnet_href) retrieve $subnet_response

          $subnet_ids << $subnet_response["body"]["resource_uid"]
        end

        $o_subnet_ids = to_s($subnet_ids)
        $o_account_id = $account_id
